 ...
 */
export function generateMonthGrid(monthDate: Date, weekStartsOn = DEFAULT_WEEK_STARTS_ON) {
  ...
  for (let week = 0; week < 6; week++) {
    ...
    const anyInMonth = weekRow.some(d => d.getMonth() === monthDate.getMonth());
    weeks.push(weekRow);
    if (!anyInMonth) break;
  }
 ...
 */
export function getWeekNumberInMonth(date: Date, weekStartsOn = DEFAULT_WEEK_STARTS_ON): number {
  const { weeks } = generateMonthGrid(date, weekStartsOn);
  for (let i = 0; i < weeks.length; i++) {
    if (weeks[i].some(d => d.getFullYear() === date.getFullYear() &&
      ...
      const firstIndex = weeks.findIndex(w => w.some(dd => dd.getMonth() === date.getMonth()));
 ...
 */
export function startOfWeekForDate(date: Date, weekStartsOn = DEFAULT_WEEK_STARTS_ON): Date {
